...
setwd("/Users/Jujuba/Desktop/extra_view/Nup98-DBPs_extraview")
```
...
Marchese F, Grossi E, Mar?n-B?jar O, Bharti S, Raimondi I, Gonz?lez J, Mart?nez-Herrera D, Athie A, Amadoz A, Brosh R, et al. A Long Noncoding RNA Regulates Sister Chromatid Cohesion. Mol Cell [Internet] 2016 [cited 2017 Jun 19]; 63:397-407. Available from: http://www.ncbi.nlm.nih.gov/pubmed/27477908
...
```{r, message=FALSE,results='hide',warning=FALSE}
RHA_RIP <- read_csv("/Users/Jujuba/Desktop/extra_view/Nup98-DBPs_extraview/RIP-seq/RIP-DHX9/RHA_RIP_genesAnno.csv", col_types = cols(X1 = col_skip()))
...
#Make sure you have unzipped the file before loading here
DDX3wtCLIP_ensemblGenes <- read_delim("/Users/Jujuba/Desktop/extra_view/Nup98-DBPs_extraview/RIP-seq/GSE70801-RIP DDX3/DDX3wtCLIP_ensemblGenes", "\t", escape_double = FALSE, trim_ws = TRUE, skip = 1)
```
...
```{r, message=FALSE,results='hide'}
DDX21wtCLIP_ensemblGenes <- read_delim("/Users/Jujuba/Desktop/extra_view/Nup98-DBPs_extraview/RIP-seq/GSE56802-RIP DDX21/DDX21wtCLIP_ensemblGenes", "\t", escape_double = FALSE, trim_ws = TRUE, skip = 1)
```
...
```{r, message=FALSE,results='hide'}
N98RIP_genes <- read_csv("/Users/Jujuba/Desktop/extra_view/Nup98-DBPs_extraview/RIP-seq/GSE67963-RIP N98/GSE67963_significance_calls_genes.csv")
...
## Loading and formatting ChIP-seq datasets

Let's read the bed files containing ChIP-seq peaks for our proteins of interest:
You can use the steps below, to reproduce the analysis or just load the file "all ChIP peaks as GRanges.RData" and skip the next 3 code chuncks.
```{r}
...
If you'd like to see the peak locations on the chromosomes run the next line
```{r, message=FALSE, warning=FALSE,fig.height=15, fig.width=20}
covplot(peaks)
```
## Distribution of ChIP-seq peaks in relation to genomic features

We can annotate these peaks and look at what genomic features they interact with:
```{r}
...
It looks like Nup98 and its interacting DBPs have a fair amount of their peaks bound to promoter regions. Leks look at the distribution of these peaks in relation to the TSS.
...
To make all plots at once let's prepare a list of tagmatrices. You can run the code below if you have more than 8GB of RAM or just load the file "all ChIP peaks tagMatrix list TSS 10k.RData" instead and skip the next code chunck.
```{r}
# Of note, you may need a computer with more than 8GB of RAM in order to run the function below.
tagMatrixList <- lapply(peaks, getTagMatrix, windows=promoter)
```

If you ran the code yourself and would like to save a file with all peaks as GRanges and a file with the tagMatrices list you can run the code below. This is how the files "all ChIP peaks as GRanges.RData" and "all ChIP peaks tagMatrix list TSS 10k.RData" mentioned above were created originally.
```{r}
...
Indeed Nup98 and its interacting DBPs have peaks that overlap around the TSS of genes, close to promoter regions. On the other hand, the control DBP, DDX11, that does not bind Nup98 shows a different distribution of peaks with no enrichment around TSS.

#### Of note:

Nup98 DamID-seq experiments have much lower resolution than the ChIP-seq experiments done for DBPs. Therefore we would expect that the Nup98 peaks have a large width, while theDBP peaks are narrower.
We can check this assumption by looking at the average peak width for each dataset.
...
## Testing the overlap of between Nup98 and DBP peaks
...
First we'll look at full length Nup98 compared to the negative control DDX11:
...
We can also test if any association is dependent on the peak position:
```{r}
lz <- localZScore(pt=pt, A=peaks[[1]], B=peaks[[5]])
plot(lz)
```

Now let's have a look at the overlap between the 2 Nup98 DamID datasets:
```{r,fig.height=9,fig.width=8}
...
Now let's repeat the same analysis looking at overlaps between a nucleoplasmic Nup98 truncation (residues 1-504) and its interacting DBPs (DDX5 and DDX21) or a control non0interacting DBP (DDX11). We'll perform all the tests and then add all the corresponding plots.
...
Now for the results. Let's begin with the control DDX11
```{r}
pt5
plot(pt5)
```
...
Now for the Nup98 truncation and Nup98 FL overlap:
```{r}
pt6
plot(pt6)
```
...
Up next the nucleoplasmic Nup98 and DDX21.
```{r}
pt7
plot(pt7)
```
...
plot(lz)
```
Finally the nucleoplasmic Nup98 and DDX5
```{r}
pt8
plot(pt8)
```
...
We need to adjust the obtained p-values to account for the multiple comparisons performed above. 
Then, we can sumarize the results above in a table below:
```{r}
...
Since we saw previously that Nup98 and its interacting DBPs preferentially bind to regions close to gene TSS, we can test if the overlap between Nup98 and DBPs is larger if we use only the peaks from promoter regions.
...
Now let's repeat the overlap tests with this subset of peaks. We'll need to change the function here so that the random peaks that are created for comparison can only be generated from promoter regions of the genome.
```{r}
pta <- permTest(A=Bound_promoters[[1]], ntimes=1000, randomize.function=resampleRegions, universe=promoter, evaluate.function=numOverlaps, B=Bound_promoters[[5]], verbose=FALSE, count.once = TRUE)
...
We'll skip doing all the plots again and just go directly to the table with the data below:
```{r}
Nup98_construct <- c(rep("Full-length",3),rep("Nucleoplasmic_1-504",3))
DBP <- rep(c("DDX11","DDX21","DDX5"),2)
Nup98_interactor <- rep(c("no","yes","yes"),2)
...
### Of Note:
These methods can show some variability of results given the randomness involved in generating the control datasets. 
Let's compare another method to detect overlap. The most conservative results are the ones shown in the manuscript.
...
The results above are quite similar to the previous ones. Let's repeat it for the promoter regions peaks.
We need to save the GRanges as bed files to do that.
```{r}
setwd("/Users/Jujuba/Desktop/extra_view/Nup98-DBPs_extraview/ChIPseq/bed promoters")
...
Unfortunately, this package does not allow us to restrict the permutation region to the promoters. We'll have to ignore the p-values obtained since they'll refer to permutations with the entire genome, consequently inflating any overlap siven that we have previously subset the peaks from similar genomic regions. However, the percentage overlap value should still be valid.
```{r}
...
The overlap percentage here is a bit smaller. From the packages documentations this package appears to require a larger number of bases overlapping in two genomic intervals to consider their overlap as true.
